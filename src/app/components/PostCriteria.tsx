...
const PostCriteria = ({ setPostFilter, purpose, setIsOpen, isOpen }: any) => {
  ...
  const handleOptionSelect = (value: any) => {
    if (questions[currentStep].type === "checkbox") {
      ...
      const currentValues: any = Array.isArray(currentResponse)
        ? currentResponse
        : [];
              ...
              <div key={optionText} className="flex items-center space-x-2">
                <Checkbox
                  ...
                  checked={
                    ...
                    currentResponse.includes(optionText)
                  }
                  onCheckedChange={() => {
                    if (option.title) {
      ...
      <RadioGroup
        ...
        onValueChange={(optionText) => {
          const selectedOption = options.find(
            (option: any) =>
              (option.title ? option.title : option) === optionText
          );

          if (selectedOption?.title) {
            console.log("3333333333333333");
            console.log(
              "sean_log selectedOption.value: " + selectedOption.value
            );
    ...
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      ...
      <DialogContent className="sm:max-w-[425px]">
        {showSequence ? (
          <>
            <DialogHeader>
              <DialogTitle>{questions[currentStep].prompt}</DialogTitle>
            </DialogHeader>
            ...
            <div className="mt-4 space-y-6">
              ...
              <div className="flex justify-end space-x-2">
                ...
                {currentStep < questions.length - 1 ? (
                  <Button onClick={handleNext} disabled={!isResponseValid()}>
                    Next
                  </Button>
                ) : (
                  <Button onClick={handleSubmit} disabled={!isResponseValid()}>
                    Submit modal
                  </Button>
                )}
              </div>
